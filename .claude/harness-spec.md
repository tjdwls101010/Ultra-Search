...
`gpt-5.6-sol`(high) 리뷰가 P1 3건·P2 4건을 짚었고 전부 반영했다: 자식이 있는 단일 런의 커서가 정수라 자식 스트림을 매번 다시 재생하던 것(기존 버그, 이 변경이 겨냥한 경로라 함께 고침 — 자식이 생기면 JSON 커서), dict가 아닌 도구 인자에서 progress가 크래시하던 것, 미지 블록이 progress에서 사라지고 `answer`/`says`를 stop reason이 아니라 파싱된 호출 유무로 가르던 것, 대상 문자열의 개행이 `run.completed` 줄을 위조할 수 있던 것, help가 인자를 전부 보여준다고 과장하던 것. 테스트 189 → 193개.

보고만 하고 안 고친 것: `out=NB`가 바이트가 아니라 문자 수다(이 변경 전부터, `tests/test_events.py`의 바이트 테스트도 문자 수를 센다). 출력 형식을 바꾸는 일이라 별도 결정으로 미룬다.
