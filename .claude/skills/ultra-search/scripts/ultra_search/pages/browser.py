"""The page snippets: fetching, opening a tab, reading sitemaps and links, in the user's browser.
...
from ultra_search.pages.discover import origin as origin_of
...
def run_snippet(name: str, args: dict, *, timeout: float = repl.REPL_HARD_LIMIT + 15) -> list[dict]:
    return repl.run_code(build_code(name, args), timeout=timeout)
